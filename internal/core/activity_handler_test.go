...
// setupActivityHandlerWithDeliverable はDeliverableHandler付きのセットアップ
func setupActivityHandlerWithDeliverable(t *testing.T) (*ActivityHandler, *DeliverableHandler, string, func()) {
	t.Helper()

	tmpDir, err := os.MkdirTemp("", "zeus-activity-ref-test")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	...
	return actHandler, delHandler, zeusPath, cleanup
}

func TestActivityHandlerAddWithRelatedDeliverables(t *testing.T) {
	actHandler, delHandler, zeusPath, cleanup := setupActivityHandlerWithDeliverable(t)
	defer cleanup()
	ctx := context.Background()

	// 先に Objective と Deliverable を作成
	fs := yaml.NewFileManager(zeusPath)
	objHandler := NewObjectiveHandler(fs, nil)
	objResult, err := objHandler.Add(ctx, "Test Objective")
	if err != nil {
		t.Fatalf("failed to add objective: %v", err)
	}

	delResult, err := delHandler.Add(ctx, "Test Deliverable", WithDeliverableObjective(objResult.ID))
	if err != nil {
		t.Fatalf("failed to add deliverable: %v", err)
	}

	// 存在する Deliverable を参照する Activity を作成（成功）
	result, err := actHandler.Add(ctx, "Activity with valid deliverable",
		WithActivityRelatedDeliverables([]string{delResult.ID}))
	if err != nil {
		t.Fatalf("Add with valid deliverable should succeed: %v", err)
	}
	if !result.Success {
		t.Error("Add should succeed with valid deliverable reference")
	}

	// 存在しない Deliverable を参照する Activity を作成（エラー）
	_, err = actHandler.Add(ctx, "Activity with invalid deliverable",
		WithActivityRelatedDeliverables([]string{"del-nonexistent"}))
	if err == nil {
		t.Error("Add with non-existent deliverable should fail")
	}
}

func TestActivityHandlerAddWithNodeDeliverableIDs(t *testing.T) {
	actHandler, delHandler, zeusPath, cleanup := setupActivityHandlerWithDeliverable(t)
	defer cleanup()
	ctx := context.Background()

	// Objective と Deliverable を作成
	fs := yaml.NewFileManager(zeusPath)
	objHandler := NewObjectiveHandler(fs, nil)
	objResult, err := objHandler.Add(ctx, "Test Objective")
	if err != nil {
	...
	// 存在する Deliverable を参照する Node を持つ Activity を作成（成功）
	nodes := []ActivityNode{
		{ID: "n1", Type: ActivityNodeTypeAction, Name: "Test Action", DeliverableIDs: []string{delResult.ID}},
	}
	result, err := actHandler.Add(ctx, "Activity with valid node deliverable",
		WithActivityNodes(nodes))
	if err != nil {
		t.Fatalf("Add with valid node deliverable should succeed: %v", err)
	}
	if !result.Success {
		t.Error("Add should succeed with valid node deliverable reference")
	}

	// 存在しない Deliverable を参照する Node を持つ Activity を作成（エラー）
	invalidNodes := []ActivityNode{
		{ID: "n2", Type: ActivityNodeTypeAction, Name: "Invalid Action", DeliverableIDs: []string{"del-nonexistent"}},
	}
	_, err = actHandler.Add(ctx, "Activity with invalid node deliverable",
		WithActivityNodes(invalidNodes))
	if err == nil {
		t.Error("Add with non-existent node deliverable should fail")
	}
}

func TestActivityHandlerUpdateWithReferenceIntegrity(t *testing.T) {
	actHandler, delHandler, zeusPath, cleanup := setupActivityHandlerWithDeliverable(t)
	defer cleanup()
	ctx := context.Background()
	...
	// Activity を作成
	actResult, err := actHandler.Add(ctx, "Test Activity")
	if err != nil {
		t.Fatalf("failed to add activity: %v", err)
	}

	// 存在する Deliverable で更新（成功）
	err = actHandler.Update(ctx, actResult.ID, map[string]any{
		"related_deliverables": []string{delResult.ID},
	})
	if err != nil {
		t.Fatalf("Update with valid deliverable should succeed: %v", err)
	}

	// 存在しない Deliverable で更新（エラー）
	err = actHandler.Update(ctx, actResult.ID, map[string]any{
		"related_deliverables": []string{"del-nonexistent"},
	})
	if err == nil {
		t.Error("Update with non-existent deliverable should fail")
	}
}

func TestActivityHandlerAddNodeWithReferenceIntegrity(t *testing.T) {
	actHandler, delHandler, zeusPath, cleanup := setupActivityHandlerWithDeliverable(t)
	defer cleanup()
	ctx := context.Background()
	...
	// 存在する Deliverable を参照する Node を追加（成功）
	validNode := ActivityNode{
		ID:             "n1",
		Type:           ActivityNodeTypeAction,
		Name:           "Valid Action",
		DeliverableIDs: []string{delResult.ID},
	}
	err = actHandler.AddNode(ctx, actResult.ID, validNode)
	if err != nil {
		t.Fatalf("AddNode with valid deliverable should succeed: %v", err)
	}

	// 存在しない Deliverable を参照する Node を追加（エラー）
	invalidNode := ActivityNode{
		ID:             "n2",
		Type:           ActivityNodeTypeAction,
		Name:           "Invalid Action",
		DeliverableIDs: []string{"del-nonexistent"},
	}
	err = actHandler.AddNode(ctx, actResult.ID, invalidNode)
	if err == nil {
		t.Error("AddNode with non-existent deliverable should fail")
	}
}
