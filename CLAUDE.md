...
make dashboard-dev               # Vite 開発サーバー起動（HMR）
go run . dashboard --dev         # Go サーバー起動（CORS 有効）

# 統合ビルド
make build-all                   # SvelteKit + Go 統合ビルド
make dashboard-clean             # ダッシュボードビルド成果物削除
```
...
### ダッシュボードモジュール (internal/dashboard/ + zeus-dashboard/)

| モジュール | 責務 |
|-----------|------|
| Server | HTTP サーバー管理、静的ファイル配信、CORS ミドルウェア |
| Handlers | REST API ハンドラー（/api/status, /api/tasks, /api/graph, /api/predict, /api/events） |
...
# Phase 5 (ダッシュボード)
zeus dashboard [--port 8080] [--no-open] [--dev]    # Web ダッシュボードを起動
```

### ダッシュボード機能

Web ブラウザでプロジェクト状態を可視化（SvelteKit + Factorio 風 UI）:
...
- `--dev` - 開発モード（CORS 有効、Vite Dev Server との連携用）
