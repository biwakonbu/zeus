...
| パネル | 機能 |
|--------|------|
| OverviewPanel | プロジェクト名、説明、健全性、進捗バー |
| StatsPanel | 完了/進行中/未着手/ブロック中のタスク数を集計 |
| TasksPanel | 全タスクをテーブル形式で表示（ステータス色分け） |
| GraphPanel | Mermaid.js による依存関係グラフ |
| PredictionPanel | 完了日予測、リスク予測、ベロシティ分析 |
...
- 接続状態インジケータ（ヘッダーに表示）
- 接続切断時は自動再接続（最大10回）
- SSE 非対応環境では 5 秒間隔ポーリングにフォールバック
...
5. **リアルタイムモニタリング**: プロジェクトの変更をリアルタイムで監視

### 7.7 Factorio 風ビューワー（VIEWER モード）

ダッシュボードには「神の視点」で大量のタスクを俯瞰できる Factorio 風ビューワーが搭載されています。画面右上の「VIEWER」ボタンで切り替え可能です。
